...
public sealed class DiscordRpcClient : IDisposable
{
    ...
    /// <summary>
    /// The logger used this client, and its associated components. <see cref="ILogger"/> are not called safely and can come from any thread. It is up to the <see cref="ILogger"/> to account for this and apply appropriate thread safe methods.
    /// </summary>
    ...
    /// <summary>
    /// Called when an error has occured during the transmission of a message. For example, if a bad Rich Presence payload is sent, this event will be called explaining what went wrong.
    /// <para>If <see cref="AutoEvents"/> is true then this event will execute on a different thread. If it is not true however, then this event is not invoked until <see cref="Invoke"/> and will be on the calling thread.</para>
    /// </summary>
    ...
    public DiscordRpcClient(string applicationID, ILogger? logger) : this(applicationID, -1, logger) { }
    ...
    public MessageBase[] Invoke()
    {
        if (AutoEvents)
        {
            Logger?.LogError("Cannot Invoke client when AutomaticallyInvokeEvents has been set.");
            return [];
    ...
    private void ProcessMessage(MessageBase? message)
    {
        ...
        switch (message.Type)
        {
            ...
            default:
                Logger?.LogError("Message was queued with no appropriate handle! {}", message.Type);
                break;
        }
    }
    #endregion
    ...
    /// <summary>
    /// Respond to a Join Request. All requests will timeouts after 30 seconds.
    /// <para>Because of the 30 seconds timeout, it is recommended to call <seealso cref="Invoke"/> faster than every 15 seconds to give your users adequate time to respond to the request.</para>
    /// </summary>
    ...
    public void Respond(ulong userID, bool acceptRequest)
    {
        ...
        _connection.EnqueueCommand(new RespondCommand
        {
    ...
    public void SetPresence(RichPresence? presence)
    {
        ...
        if (!IsInitialized)
            Logger?.LogWarning("The client is not yet initialized, storing the presence as a state instead.");
        ...
        else
        {
            ...
            if (presence.HasParty() && presence.Party != null && presence.Party.Max < presence.Party.Size)
            ...
            if (presence.HasSecrets() && !presence.HasParty())
                Logger?.LogWarning("The presence has set the secrets but no buttons will show as there is no party available.");
    ...
    public bool RegisterUriScheme(string? steamAppID = null, string? executable = null)
    {
        SchemeInfo info = new()
        {
            ...
            ExecutablePath = executable ?? Environment.ProcessPath ?? ""
        };

        return HasRegisteredUriScheme = UriScheme.Register(info, Logger);
    }
    ...
    public void SetSubscription(EventType type)
    {
        ...
        {
            Logger?.LogWarning("Client has not yet initialized, but events are being subscribed too. Storing them as state instead.");
        }
